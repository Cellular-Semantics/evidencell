...
def check_at_f1_attribution(doc: dict) -> list[str]:
    """Reject AT evidence whose ``best_mapping_level`` disagrees with the
    target edge's accession-derived level.

    A small number of older edges recorded the parent supertype's F1 on
    a cluster-level edge (or the subclass's on a supertype edge). The
    audit/correction pass in ``evidencell.at_f1_attribution`` cleaned up
    existing instances; this check prevents new ones from being added.

    Returns a list of error strings; empty = OK. Items with no
    ``best_mapping_level`` recorded are passed through (the field is
    optional in the schema), but a recorded value MUST match the
    accession-derived target level.
    """
    if not isinstance(doc, dict):
        return []

    # Local copy of the prefix → level map to avoid a circular import
    # back into evidencell.at_f1_attribution.
    PREFIX_TO_LEVEL = {
        "_CLUS_": "CLUSTER",
        "_SUPT_": "SUPERTYPE",
        "_SUBC_": "SUBCLASS",
    ...
    nodes_by_id = {
        n["id"]: n for n in (doc.get("nodes") or [])
    ...
    for edge in doc.get("edges") or []:
        ...
        target_level = None
        for pfx, lvl in PREFIX_TO_LEVEL.items():
            if pfx in target_acc:
                target_level = lvl
                break
        if not target_level:
            continue
        for ev in edge.get("evidence") or []:
            ...
            if not bml:
                continue
            if str(bml).upper() != target_level.upper():
                errors.append(
                    f"edge {edge.get('id', '<unnamed>')}: AT evidence has "
                    f"best_mapping_level={bml!r} but target {target_acc} is "
                    f"at level {target_level}. "
                    f"Record F1 at the target's level, or attach the AT "
                    f"evidence to an edge whose target is at level {bml}. "
                    f"See workflows/annotation-transfer.md."
                )
    return errors
