...
**Notes.** Sst::Tac1 intersectional genetics targets bistratified cells that overwhelmingly target fast-spiking interneurons (Chamberland et al. 2024 [9]).
...
|---|---|---|---|---|---|
| 1 | 0216 Sst Gaba_3 [CS20230722_SUPT_0216] (supertype) | — | 🔴 LOW | Tac1 CONSISTENT · Sst CONSISTENT · Pvalb DISCORDANT | Speculative |
...
- Annotation transfer (PARTIAL): MapMyCells local transfer of Yao 2021 (GEO:GSE185862) SSv4 Sst subclass hippocampal cells (n=273) onto WMBv1 maps 83/273 cells to SUPT_0216 (F1=0.488, target_purity=1.0, supertype level). A separate Pvalb SSv4 subclass transfer (n=66 HIP cells) yields only 6/66 cells to SUPT_0216 (F1=0.053, target_purity=0.036); the Pvalb population maps predominantly to Pvalb chandelier (SUPT_0204, F1=0.612) and Pvalb Gaba_2 (SUPT_0206, F1=0.324). The partial Sst signal lends weak support; the absent Pvalb signal reflects the Sst-subclass placement of this supertype.

**Marker evidence provenance**

- **Pvalb** [5][6][7][8]: Evidence spans transcript-level and protein-level methods across four studies. Dannenberg et al. 2017 [4] classifies bistratified cells within the PV+ interneuron group. Tzilivaki et al. 2023 [7] explicitly enumerates morphological subtypes:
...
> — Tzilivaki et al. 2023, Classification Schemes and Methodological Approaches · [7] <!-- quote_key: 221276443_e917908b -->
  ...
  Cell-type specificity for bistratified cells is confirmed in [7] and [6] via morphological subtype classification and Cre-driver targeting. Que et al. 2021 [8] highlights a key constraint on Pvalb-based classification:
...
> — Que et al. 2021, unknown · [8] <!-- quote_key: 230508306_e8cc8c19 -->
  ...
  This is directly relevant to the DISCORDANT Pvalb alignment: the continuous transcriptomic landscape of PV interneurons means bistratified cells, despite expressing Pvalb, may sit transcriptomically closer to Sst-class supertypes than to Pvalb-class ones.
...
- **Tac1** [9]: Evidence from Chamberland et al. 2024 [9] only — a single primary citation. Method: Sst;;Tac1 Cre-driver with morphological and connectivity confirmation (strong cell-type specificity for this study). Because only one primary study establishes Tac1 as a bistratified-cell marker, independent confirmation is needed.

- **Soma location** [1][2][3]: Three independent studies confirm soma placement across stratum pyramidale [UBERON:0005401], stratum oriens [UBERON:0005383], and stratum radiatum [UBERON:0005402] of CA1. Additional context from Dannenberg et al. 2017 [4]:
...
> — Dannenberg et al. 2017, Classification Schemes and Methodological Approaches · [4] <!-- quote_key: 38778375_462ec931 -->
  ...
  Ekins et al. 2020 [5] further characterises PV+ interneuron subtypes:
...
- **Pvalb marker — DISCORDANT**: Bistratified cells co-express Pvalb and Sst, but SUPT_0216 belongs to the Sst subclass; Pvalb is not among supertype markers, and the precomputed mean for Pvalb is 1.48 (low). The Sst-class placement captures the Sst component of bistratified identity but misses the Pvalb component entirely. The continuous transcriptomic landscape of PV interneurons noted in Que et al. 2021 [8] likely means bistratified cells sit transcriptomically closer to OLM cells than to PV basket cells. *(note: this interpretation of relative transcriptomic proximity is consistent with [8] but not directly stated in the facts.)*
...
- **Prosubiculum and posterior amygdala signal**: SUPT_0216 carries 259 prosubiculum cells and 780 posterior amygdala cells. Posterior amygdala is anatomically distant from CA1 *(note: distant region — stronger counter-evidence; the classical type may still be a subtype of this T-type but the posterior amygdala population is not the CA1 bistratified population specifically)*.
- **DISTRIBUTED_ACROSS_CLUSTERS**: SUPT_0216 contains at least three classical hippocampal types: OLM cells (Sst+/Chrna2+), bistratified cells (Sst+/Pvalb+/Tac1+), and HS cells (Sst+, long-range projecting). These are not separable at supertype level.
- **Annotation transfer weak for PV population**: MapMyCells of Yao 2021 Pvalb SSv4 subclass (n=66 HIP cells) onto WMBv1 yields F1=0.053 at SUPT_0216 with target_purity=0.036. The Pvalb population predominantly maps to Pvalb-subclass supertypes (SUPT_0204 and SUPT_0206), not to SUPT_0216. The Yao 2021 'Pvalb' SSv4 label encompasses PV basket, axo-axonic, and bistratified cells without morphological resolution.
...
- Targeted `cite-traverse` for "Tac1 bistratified hippocampus" or "substance P bistratified interneuron hippocampus" to confirm Tac1 as a bistratified marker independently of the single Chamberland 2024 citation [9]. Independent confirmation would strengthen the CONSISTENT alignment verdict for `marker_Tac1`.
- Cluster-level separation of OLM, bistratified, and HS cell matches within SUPT_0216 would directly address the DISTRIBUTED_ACROSS_CLUSTERS caveat.
...
*Existing annotation transfer status:* A SUPERTYPE-level MapMyCells transfer is recorded using Yao 2021 (GEO:GSE185862) SSv4 data. The Sst SSv4 subclass maps 83/273 HIP cells to SUPT_0216 with F1=0.488; the Pvalb SSv4 subclass yields only F1=0.053 (6/66 cells). This provides partial support but cannot resolve cluster-level identity and does not use a morphologically confirmed bistratified cell source.
...
- **Expected output**: `LiteratureEvidence` item on the `marker_Tac1` property comparison
- **Resolves**: Open question 2; removes single-citation caveat on Tac1 marker

---

## Open questions

1. Can bistratified cells be separated from OLM and HS cells at WMBv1 cluster level within Sst Gaba_3 supertype [CS20230722_SUPT_0216]?
2. Is Tac1 confirmed as a bistratified cell marker in independent primary studies beyond Chamberland et al. 2024 [9]?
...
| [9] | Chamberland et al. 2024 | [38640347](https://pubmed.ncbi.nlm.nih.gov/38640347/) | Sst marker; Tac1 marker; Sst neuropeptide |
