...
**Classical type definition.** avpv_th_neuron is defined on a CLASSICAL_NEUROCHEMICAL basis: defining marker Th (dopaminergic phenotype) [1], [2], [4] with Kiss1 co-expression [3], [5]; soma location in AVPV [MBA:272]; FEMALE_BIASED sex dimorphism (2–4× more TH+ in females) [1], [4]. Multi-modal evidence: IHC for TH protein, ISH for Kiss1 mRNA, transgenic reporters.

**Atlas mapping query.** Candidates retrieved from CCN20230722 at ranks 0 (cluster) and 1 (supertype) using metadata-based scoring. Full scoring rules: `workflows/map-cell-type.md`.

**Property alignment.** Each defining property compared via the `property_comparisons` schema; alignments graded CONSISTENT / APPROXIMATE / DISCORDANT / NOT_ASSESSED. Atlas values from precomputed cluster expression and MERFISH spatial registration.
...
**Primary mapping:** AVPV TH neuron → 1915 PVpo-VMPO-MPN Hmx2 Gaba_5 [CS20230722_CLUS_1915] at MODERATE confidence. Key support: ATLAS_METADATA (Th=6.6, Kiss1=2.51, MFR=0.02, nt_type=Dopa). Key caveats: `LOW_CELL_COUNT` (CLUS_1915 has n=3–5 MERFISH cells) and `TAXONOMY_LEVEL_MISMATCH` (avpv_th_neuron and avpv_kiss1_neuron both map to CLUS_1915 — most AVPV TH cells co-express Kiss1).

No Cell Ontology term currently assigned. The classical type overlaps with the AVPV/PeN kisspeptin neuron (CL:4023123) but a dedicated CL term for the AVPV dopaminergic / TH+ population may be warranted.
