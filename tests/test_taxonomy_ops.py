...
# ── AT artifact producer (Phase 1 commit 5) ─────────────────────────────────


def _make_run_dir(tmp_path: Path, taxonomy_id: str = "TESTTX") -> Path:
    """Build a minimal MapMyCells run dir with f1_matrix.csv + manifest.yaml."""
    ...
    (run_dir / "f1_matrix.csv").write_text(
        ...
        "Htr3a-OLM,cluster,other,5,0.50,0.30,0.40,0.50,0.50\n"
    )
    manifest = {
        "id": "at_run_test",
        "record_type": "AnnotationTransferRun",
        "target_taxonomy_id": taxonomy_id,
        "source_dataset_accession": "GEO:GSETEST",
    }
    (run_dir / "manifest.yaml").write_text(yaml.safe_dump(manifest))
    return run_dir


def _make_taxonomy_db(tmp_path: Path, taxonomy_id: str = "TESTTX") -> Path:
    """Build a minimal taxonomy SQLite DB with two named nodes for resolution."""
    import sqlite3
    db_path = tmp_path / "kb" / "taxonomy" / taxonomy_id / f"{taxonomy_id}.db"
    db_path.parent.mkdir(parents=True)
    con = sqlite3.connect(db_path)
    con.execute(
        "CREATE TABLE nodes (node_id TEXT PRIMARY KEY, label TEXT, "
        "taxonomy_level TEXT, taxonomy_rank INTEGER)"
    )
    con.executemany(
        "INSERT INTO nodes(node_id, label, taxonomy_level, taxonomy_rank) "
        "VALUES (?, ?, ?, ?)",
        [
            ("CS_TESTTX_SUPT_0216", "0216 Sst Gaba_3", "supertype", 1),
            ("CS_TESTTX_CLUS_0769", "0769 Sst Gaba_3", "cluster", 0),
        ],
    )
    con.commit()
    con.close()
...
def test_extract_at_f1_artifact_basic(tmp_path, monkeypatch):
    """End-to-end producer run: filter f1_matrix, resolve labels, write artifact."""
    import evidencell.paths as paths_mod
    monkeypatch.setattr(paths_mod, "repo_root", lambda: tmp_path)
    monkeypatch.setattr(
        paths_mod, "taxonomy_db_path",
        lambda tid: tmp_path / "kb" / "taxonomy" / tid / f"{tid}.db",
    )

    run_dir = _make_run_dir(tmp_path)
    _make_taxonomy_db(tmp_path)

    result = extract_at_f1_artifact(
        run_dir=run_dir,
        classical_node_id="olm_test",
        source_cluster_label="Sst-OLM",
        region="hippocampus",
        f1_floor=0.2,
    )

    assert result["taxonomy_id"] == "TESTTX"
    assert result["run_id"] == "at_run_test"
    assert result["hits"] == 2  # supertype + cluster; the 0.07 row is below floor
    assert result["unresolved"] == []

    artifact_path = tmp_path / result["artifact_path"]
    assert artifact_path.exists()
    artifact = json.loads(artifact_path.read_text())
    assert artifact["classical_node_id"] == "olm_test"
    assert artifact["taxonomy_id"] == "TESTTX"
    assert artifact["source_run_id"] == "at_run_test"
    assert artifact["source_cluster_label"] == "Sst-OLM"
    assert artifact["f1_floor"] == 0.2

    accs = {h["target_accession"] for h in artifact["hits"]}
    assert accs == {"CS_TESTTX_SUPT_0216", "CS_TESTTX_CLUS_0769"}


def test_extract_at_f1_artifact_filters_by_floor(tmp_path, monkeypatch):
    """F1 floor drops hits below the threshold."""
    import evidencell.paths as paths_mod
    monkeypatch.setattr(paths_mod, "repo_root", lambda: tmp_path)
    monkeypatch.setattr(
        paths_mod, "taxonomy_db_path",
    ...
    result = extract_at_f1_artifact(
        ...
        source_cluster_label="Sst-OLM",
        region="hippocampus",
        f1_floor=0.6,  # only the supertype 0.67 hit qualifies
    )
    assert result["hits"] == 1


def test_extract_at_f1_artifact_filters_by_levels(tmp_path, monkeypatch):
    """target_levels restricts to the listed levels."""
    import evidencell.paths as paths_mod
    monkeypatch.setattr(paths_mod, "repo_root", lambda: tmp_path)
    monkeypatch.setattr(
        paths_mod, "taxonomy_db_path",
    ...
    result = extract_at_f1_artifact(
        ...
        region="hippocampus",
        f1_floor=0.2,
        target_levels=["supertype"],
    )
    assert result["hits"] == 1


def test_extract_at_f1_artifact_unresolved(tmp_path, monkeypatch):
    """target_name not present in taxonomy DB lands in `unresolved`."""
    import evidencell.paths as paths_mod
    monkeypatch.setattr(paths_mod, "repo_root", lambda: tmp_path)
    monkeypatch.setattr(
        paths_mod, "taxonomy_db_path",
    ...
    # Create DB with a different label than what's in f1_matrix
    import sqlite3
    db_path = tmp_path / "kb" / "taxonomy" / "TESTTX" / "TESTTX.db"
    db_path.parent.mkdir(parents=True)
    con = sqlite3.connect(db_path)
    con.execute(
        "CREATE TABLE nodes (node_id TEXT, label TEXT, taxonomy_level TEXT, taxonomy_rank INTEGER)"
    )
    con.execute(
        "INSERT INTO nodes VALUES (?, ?, ?, ?)",
        ("CS_X", "Different Label", "supertype", 1),
    )
    con.commit()
    con.close()
    ...
    assert len(result["unresolved"]) == 2  # supertype + cluster row both unresolved


def test_extract_at_f1_artifact_missing_run_dir(tmp_path):
    """Missing f1_matrix.csv raises FileNotFoundError."""
    bad_dir = tmp_path / "nonexistent"
    with pytest.raises(FileNotFoundError, match="f1_matrix.csv"):
        extract_at_f1_artifact(
            run_dir=bad_dir,
            classical_node_id="x",
            source_cluster_label="x",
            region="x",
        )


class TestAtExtractF1CLI:
    def test_at_extract_f1_help(self):
        import subprocess
        result = subprocess.run(
