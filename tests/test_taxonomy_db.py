...
def test_find_candidates_region_filter_drops_with_anat_elsewhere(populated_db):
    ...
    queried region is hard-dropped (programmatic only at Phase 1 follow-up;
    LLM adjacency wiring reverted)."""
    db, _, _ = populated_db
    ...
    assert node_id not in result_ids, (
        "Region-mismatched candidate must be dropped programmatically"
    )
...
def test_cmd_find_candidates_default_top_k_is_ten():
    """The CLI default for top_k is 10 (Phase 1 follow-up D bump from 5)."""
    import inspect
    from evidencell import taxonomy_db as tdb
