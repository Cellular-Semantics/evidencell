...
- [x] **AnnotationTransfer graph integration** ([#32](https://github.com/Cellular-Semantics/evidencell/issues/32)) — thin-registry option implemented: `kb/annotation_transfer_runs/index.yaml` generated by `just register-at-run` (`build_at_index()`); `check_run_refs()` in validate.py validates `run_ref` fields against the index; pre-write hook blocks unregistered `run_ref` values; `at_back_index()` returns run→citing-files on demand. `#annotation-transfer` `#schema`
...
- [x] **find-candidates: DB staleness preflight + quantitative expression scoring** ([#34](https://github.com/Cellular-Semantics/evidencell/issues/34)) — `find-candidates` now aborts with a clear error if the DB is stale (mirrors `gen-report`). `find_candidates()` accepts `expression_data` and `negative_markers`; `_cmd_find_candidates` auto-loads expression data from the YAML level matching the query rank. Scoring: ≥5.0→+2, ≥1.0→+1, ≥0.1→0 (flagged), <0.1→−2; negative markers inverted; binary +1 fallback when expression data absent. `#map-cell-type` `#find-candidates`
