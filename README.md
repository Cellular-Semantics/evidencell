...
Dev access is gated: writes to `src/`, `schema/`, and `justfile` are blocked by the pre-edit hook in curation sessions. If you need to work on code, schema, or tooling, request dev setup from the repo admin (@dosumis). Code and schema changes land through PR review against `main`.
